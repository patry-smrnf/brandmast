// src/components/dashboard/BMChartBoard.tsx
...
interface ActionData {
  ...
  action_real_start?: string | null;
  action_real_end?: string | null;
  szkolenie: boolean;
  past: boolean;
}
...
const BMChartBoard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await apiFetch<ActionData[]>(`/api/bm/actions?month=9`, {
          method: "GET",
          credentials: "include",
        });
  ...
  const dailyHours = useMemo(() => {
    ...
    data.forEach(item => {
      ...
      if (!grouped[date]) grouped[date] = { date, systemHours: 0, realHours: 0 };

      grouped[date].systemHours += calculateHours(item.action_system_start, item.action_system_end);
      grouped[date].realHours += calculateHours(item.action_real_start || '00:00:00', item.action_real_end || '00:00:00');
    });
  ...
  const typeCounts = useMemo(() => {
    ...
    data.forEach(item => {
      counts[item.shop_name] = (counts[item.shop_name] || 0) + 1;
    });
      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-zinc-900 text-white p-6">
        <div className="max-w-7xl mx-auto space-y-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              { title: 'Total Actions', value: data.length, color: 'text-violet-400' },
              { title: 'System Hours', value: dailyHours.reduce((sum, day) => sum + day.systemHours, 0).toFixed(2), color: 'text-emerald-400' },
              { title: 'Real Hours', value: dailyHours.reduce((sum, day) => sum + day.realHours, 0).toFixed(2), color: 'text-sky-400' },
            ].map((stat, i) => (
              <Card key={i} className="bg-gray-800/50 border border-gray-700/60 backdrop-blur-md shadow-lg hover:shadow-xl transition-shadow rounded-2xl">
                <CardHeader>
